...
> `HttpFilter` 클래스의 `doFilter()` 메서드는 매개변수가 이미 `HttpServletRequest`와 `HttpServletResponse` 타입으로 정의되어 있기 때문에 `Filter` 클래스와 달리 HTTP 요청과 응답을 다루기 위해서는 명시적으로 형변환할 필요가 없다.
{: .prompt-info }
