...
*[H2 Database 홈페이지](https://www.h2database.com/html/main.html){: target="_blank" } > `Cheat Sheet`*
...
*프로젝트/src/test/resources/config 폴더의 애플리케이션 설정 파일에 복사한 URL 붙여넣기*

일반적으로 프로젝트의 소스 코드는 src/main/resources/ 디렉토리에 위치하며, 이 디렉토리의 리소스가 운영 환경에서 사용된다. 그러나 테스트를 위한 리소스는 src/test/resources/ 디렉토리에 위치한다. 이 디렉토리의 리소스는 테스트 환경에서 사용되며, 테스트 코드가 실행될 때 클래스패스에 추가된다.

따라서 테스트 환경에서는 우선적으로 src/test/resources/ 디렉토리에 있는 리소스를 참조한다. 이는 테스트 중에 프로덕션 환경의 설정을 오염시키지 않고 테스트에 필요한 리소스를 관리할 수 있도록 도와준다. 이때 **이클립스에서 애플리케이션 설정 파일은 config 폴더 내에 위치해야 인식하므로 config 폴더를 추가로 생성**해야 한다.

> 인텔리제이는 애플리케이션 설정 파일이 config 폴더 내에 위치하지 않아도 인식한다.
{: .prompt-info }
