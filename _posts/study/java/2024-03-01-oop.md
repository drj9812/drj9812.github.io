...
- **추상 메서드의 집합**
	+ `static` 메서드, `default` 메서드, 상수(`static final`)를 요소로 가질 수 있음
...
- **인터페이스를 구현하는 구현체 타입의 참조 변수로 인터페이스 타입 객체를 참조할 수 있음**
	+ 인터페이스 타입의 참조 변수로 구현체 타입의 객체를 참조할 수는 없음
	+ 구현체 타입의 참조 변수는 구현체 타입의 멤버만 참조할 수 있음
...
- [남궁성의 정석코딩, "[자바의 정석 - 기초편] ch6-1 객체지향 언어", 2021-01-01](https://www.youtube.com/playlist?list=PLW2UjW795-f6xWA2_MUhEVgPauhGl3xIp){: target="_blank" }
