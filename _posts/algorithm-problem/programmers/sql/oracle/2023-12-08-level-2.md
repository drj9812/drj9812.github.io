...
```
`SUBSTR` 함수는 문자열의 일부분을 추출하는 데 사용되는 함수다. 주로 특정 문자열에서 시작 위치부터 일정 길이만큼 또는 끝까지의 문자열을 반환하는 데에 활용된다. 세 번째 인수를 지정하지 않으면 두 번째 인수에 들어간 값(시작 위치)부터 문자열의 끝까지를 반환한다.
...
집계함수는 `WHEREW`절에서 사용할 수 없으며 `SELECT`절이나 `HAVING`절에서 사용된다.

`ROUND`함수의 두 번째 인자에 1을 넣으면, 소수점 둘째자리를 반올림한다.